...
public:
  ...
  //inserts at ordered position
  DListNode<T>* insert(T& obj);//f(n) = n + 8, f(n) = O(n)
...
template<class T> DListNode<T>* DoublyLinkedList<T>::insert(T& obj){
	DListNode<T>* tempPtr = header.next;
	if(tempPtr->next != NULL)//check if we're on the trailer node
	{
		while(obj < tempPtr->obj && tempPtr->next != NULL)//if not, start searching
